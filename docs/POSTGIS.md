...
- batch writer は 1 トランザクション/`write_batch`、`INSERT INTO ... VALUES (...), (...), ...` の multi-row 形式で 1 RPC に複数行を詰める。PostgreSQL は extended protocol Bind の param 数を `Int16` (signed 16-bit) で送るため実用上限 32767 から `chunk_rows = floor((32767 - safety) / params_per_row)` を schema 確定時に算出し、`chunks(chunk_rows)` ループする (属性 6 列 + Point の典型 schema で `chunk_rows ≈ 4680`、~6500× の round-trip 削減)。chunk_rows ごとに SQL shape が変わるため `client.prepare` の事前キャッシュは持たず `tx.execute(&str, &refs)` で都度 Parse → Bind → Execute する (chunk あたり SQL 文字列の send overhead は無視できる)。Decimal128 は `PgNumeric` newtype で `ToSql` を独自実装し、binary 経路と同じバイト列を bind する。
