...
- 書き:
  - `--overwrite` で `DROP TABLE IF EXISTS <table>` → CREATE TABLE → 1 トランザクション + prepared `INSERT INTO ... VALUES ($1, ..., ST_GeomFromEWKB($N))` で行単位投入
...
- CI 上の docker postgis (`postgis/postgis:16-3.4`) で SHP / Parquet ↔ PostGIS の往復テストが緑
...
| `Decimal128(p, s)` | `numeric(p, s)` | 無損失 |
| `Decimal256(p, s)` | `numeric(p, s)` | 38 < p ≤ 76 は warn で切り詰め検討（cycle 1 では p ≤ 38 のみテスト） |
...
- `NUMERIC` → `Decimal128(38, scale)`（typmod から scale を取れたら使う、取れなければ scale=0）
...
cycle 1 で未対応の OID（`json`/`jsonb`/`uuid`/array 系）は `Error::Schema` で停止する。
...
v0.3 cycle 1 では以下が未対応:

- **COPY BINARY 自前エンコーダ + `BulkLoadWriter`**（cycle 2）
- **`--insert-mode=bulk|batch`** CLI フラグ（cycle 2）
- **`--where`/`--select`/`--query`** reader 側の絞り込み（cycle 3）
- **`--create-table=if-not-exists|always|never`** writer 側の制御（cycle 1 は `--overwrite` で DROP するだけ）
- **GIST index 自動生成オプション**（cycle 3）
- **`spatial_ref_sys` への未登録 EPSG 自動 INSERT**（cycle 3）
- **streaming reader**（cycle 1 は全件 in-memory）
...
- writer は 1 トランザクション/`write_batch`、prepared `INSERT` を毎行 execute する。cycle 2 で `CopyInSink` 経由の binary COPY に置き換え予定。
- reader は `client.query("SELECT ST_AsEWKB(geom) AS geom, ... FROM tbl", &[])` を `block_on` で呼んで `Vec<Row>` を取得し、`batch_size_hint` チャンクで `RecordBatch` を組む。streaming は cycle 2 で `query_raw` に置き換え予定。
