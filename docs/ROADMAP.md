...
- **cycle 6 — メモリベンチ + docs + 0.8.0 release**（完了）: `crates/shpx-core/src/bench_util.rs` を新設し `peak_rss_kib() -> Option<u64>` を提供 (Linux: `/proc/self/status` の `VmHWM`、その他 OS: `None`)。`procfs` crate は使わず std だけで完結 (依存最小化)。`docs/STREAMING.md` を新設し driver × streaming 戦略 × 期待 peak RSS の表、キャンセル挙動、トランザクション保持期間、バッチサイズ影響、関連実装ファイルへのリンクを 1 箇所に集約。`CHANGELOG.md` に v0.8.0 セクション、workspace `Cargo.toml` を `0.8.0` へ bump。各 driver の `benches/reader_streaming.rs` は将来 PR に分離 (Linux CI でしか計測できないため、リリース commit と独立して追加する方が PR レビューしやすい)。
