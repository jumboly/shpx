...
**完了基準**:
- [x] SQL Server 10M 行ベンチが Linux x86_64 で shpx 単独 wall-clock として公開され、tempdb 溢れずに完走する (`bench-smoke-mssql.yml` workflow、実測値は `docs/SQLSERVER.md` Benchmark 節)。当初予定していた ogr2ogr 比較 (`shpx_secs <= 1.667 * ogr_secs`) は本 cycle で完了基準から外し、絶対値判定に切替
...
- **cycle 1 — v0.4 ベンチ + LICENSE / NOTICE**: 既存 `crates/shpx-driver-sqlserver/benches/{bulk_insert,gen}.rs` を新設の `.github/workflows/bench-smoke-mssql.yml` 経由で Linux x86_64 (ubuntu-latest, `workflow_dispatch`) で実行し 10M 行 × 3-run median を取得 (実測値は `docs/SQLSERVER.md` Benchmark 節)。完了基準は当初の `shpx_secs <= 1.667 * ogr_secs` から「shpx 単独 wall-clock + tempdb 溢れなし完走」に再定義し ogr2ogr 比較を外した (`scripts/bench-vs-ogr-mssql.sh` はローカル参考 utility として温存)。`docs/SQLSERVER.md` Benchmark 節 / `CHANGELOG.md` 0.4.0 Known Issues / `docs/ROADMAP.md` v0.4 完了基準を訂正。`/Users/masa/src/shpx/LICENSE-MIT` / `/Users/masa/src/shpx/LICENSE-APACHE` / `/Users/masa/src/shpx/NOTICE` を新設 (NOTICE は libspatialite / GEOS / PROJ / SQLite / arrow-rs 等の third-party listing を aggregate、既存 `crates/shpx-driver-spatialite/NOTICE` は driver scope のまま残す)。
...
- **cycle 4 — cargo-dist + multi-platform CI**: cargo-dist (0.31 で `dist` にリブランド済み) を `.scratch/cargo-dist/bin/dist` 経由で導入し、`dist init --yes --hosting github --installer shell --target {5 triples}` で **`dist-workspace.toml`** (cargo-dist 0.31 のデフォルト形式、`[workspace.metadata.dist]` 形式の後継) を生成。targets は `aarch64-apple-darwin` / `aarch64-unknown-linux-gnu` / `x86_64-apple-darwin` / `x86_64-unknown-linux-gnu` / `x86_64-pc-windows-msvc` の 5 triples、`installers = ["shell"]`。`bundled-spatialite` feature 有効化は `crates/shpx-cli/Cargo.toml` の `[package.metadata.dist].features` で宣言 (driver の bundled-spatialite と shpx-geom/bundled-proj を implies)。`dist-workspace.toml` 側の `[dist] features` は cargo-dist 0.31 では読まれないため package metadata 側に置く。`shpx-bench-rss` は workspace 内ツールなので `[package.metadata.dist] dist = false` で配布対象外に。`dist generate` で `.github/workflows/release.yml` を生成。`.github/workflows/ci.yml` に `bundled-spatialite-smoke (macos-arm64)` / `bundled-spatialite-smoke (windows)` job を追加 (macOS は `brew install cmake`、Windows は chocolatey で `llvm`、Windows のみ `continue-on-error: true`)。Linux smoke (`bundled-spatialite-smoke (linux)`) は既存。`docs/SPATIALITE.md` の bundled 節に「macOS arm64 + Linux のみ Release、Windows / macOS x64 は best-effort」の縮退方針を表形式で明記。**build.rs 修正**: `dist build --target=<triple>` 経由では `[build-dependencies]` の geos-src OUT_DIR が host build dir (`target/<profile>/build/`) に出力される一方、shpx-driver-spatialite 自身は target build dir (`target/<triple>/<profile>/build/`) で動くため、既存の `locate_sibling_out` (sibling 走査のみ) では geos_c.h を見失う。`crates/shpx-driver-spatialite/build.rs` に `host_build_root_from_target_out_dir` を追加し、primary build_root に sibling が居なければ host build_root も走査するフォールバックを入れた。`cargo build` (`--target` なし) の従来経路は primary 走査だけで成立するため挙動変化なし。**ローカル検証**: `dist build --target=aarch64-apple-darwin --artifacts=local` で 6.5 MB の tarball 生成、`otool -L` で bundled binary の dynamic dep が macOS system フレームワーク + libz/libiconv のみ (libspatialite / libgeos / libproj への依存ゼロ) であることを確認。
...
- **SQL Server クライアント戦略 (decision tree、中期完了済み)** — `shpx-driver-sqlserver` は `tiberius` (Apache 2.0、prisma/tiberius) を使用しているが、upstream `0.12.3` には **`Daten` COLMETADATA に length バイトを 1 個多く書き出す bug** がある (`bulk_all_types_together` が `#[ignore]` 化されていた背景)。**確定済み再現条件 (2026-05-04 計測, `docs/SQLSERVER_BULK_BUG_REPRO.md`)**: `DataType::Date32` 列を含む schema で `bulk_insert` を呼ぶと、Date32 列の **直後** の列で `Invalid column type from bcp client for colid N` を踏む。最小再現は 3 列 / 1 行 (`id Int64 + created Date32 + geom`)。**修復 + 性能改善**: 2026-05-04 に `jumboly/tiberius#shpx-patches` を fork として保持し、shpx workspace `Cargo.toml` の `tiberius` dep を git fork 参照 (`rev` ピン) に切り替えた。fork に当てた upstream patch は **(1) PR #346 (Daten COLMETADATA length バイト bug 修正)**、**(2) PR #400 (LOGIN7 packet_size 設定、bulk insert throughput +42% を実証)** の 2 件。`bulk_all_types_together` の `#[ignore]` も解除済み、`bulk_repro` 35 case 全緑 + 既存統合テスト 90 件無 regression を確認。**tiberius のメンテ状況**: archived ではないが「最後の意味あるコード変更が 2025-02」「`0.12.3` release が 2024-07」「open issues 131、bulk 系 #312/#302/#322/#352/#373/#410 が長期 open」「PR #346 / #400 とも 1〜2 年 review なしで auto-close 待ち」。upstream の active 開発は事実上停止しており、shpx は fork 維持で運用継続する。**判断ツリー (今後)**:
    - **短期 — fork 維持**: `jumboly/tiberius#shpx-patches` をそのまま参照する。upstream 修正不要、保守工数 0。新たに有用な PR を見つけたら同 branch に追記して `rev` を更新する運用。
    - **中期 — upstream 復活時**: prisma/tiberius がメンテ復活、または PR #346 / #400 相当が別 PR で merge された場合は shpx 側 dep を `crates.io` 版に戻す (`Cargo.toml` 1 行差分)。
    - **長期 (v2.0 構造変更案)**: 別の tiberius bug を踏む / upstream が archived になる場合は `odbc-api` (Microsoft 純正 ODBC driver 経由) への乗り換えを検討。**配布哲学のトレードオフ**: shpx は v0.6 以降「pure Rust + 単一バイナリ + system dep ゼロ」を志向しており、odbc-api は `msodbcsql18` のホスト install を要求するため `cargo-dist` 戦略の修正が必要。`bcp` subprocess は bench-rss の prepare 用途のみの限定経路としては有用だが本番 driver には組み込まない (Rust library 哲学外)。`sqlx` は MSSQL backend を 0.7 で削除済みかつ実装が tiberius wrapper だったため代替にならない。
    - **トリガー条件**: 長期 (odbc-api) に進む合図は (a) tiberius fork で別の bulk bug を踏み修正範囲が増える、(b) tiberius repo が archived になる、(c) upstream が `crates.io` を yank する のいずれか。
...
- **RDB writer の multi-row `VALUES` batch INSERT (完了)** — 旧 `LayerWriter::write_batch` は 1 行 1 RPC を発行していたが、`INSERT INTO t (...) VALUES (?,?,...), (?,?,...), ...` で multi-row 化し round-trip を桁違いに削減した。`shpx-rdb-common::multirow_chunk_rows(params_per_row, max_params_per_rpc, safety_margin)` を共通ヘルパとして置き、SQL Server (param 上限 2100、属性 6 列で `chunk_rows ≈ 260`、~175× 削減) と PostGIS (PostgreSQL extended protocol Bind の Int16 上限 32767、`chunk_rows ≈ 4680`、~6500× 削減) の双方で同 helper を呼ぶ対称実装。bench-rss SQL Server prepare の 1h ペインに直結。SpatiaLite / GPKG はローカル SQLite で network round-trip 無しのため対象外。MySQL / Oracle が後続で追加されるときは同 helper を再利用する。トランザクション境界は 1 batch = 1 トランザクションを維持し、chunk 境界では COMMIT しない。
