...
- SQL Server 型網羅 bulk insert の真因特定 — tiberius 0.12 + SQL Server 2022 で「多列 + decimal + 連続 `varbinary(max)`」を含む staging schema で `bulk_insert` が `Invalid column type from bcp client for colid N` を返す。`crates/shpx-driver-sqlserver/tests/bulk_roundtrip.rs::bulk_all_types_together` が `#[ignore]` にしてある現象と同根。実証された切り分け: (1) `datetimeoffset` 単独 (`bulk_timestamptz_and_int64_bit_identical`) は緑、(2) `decimal` 単独 (`bulk_decimal_38_10_bit_identical`) は緑、(3) 両方 + `varbinary(max)` 列 (`payload`) + WKB(varbinary(max))/SRID(int) の staging 拡張がそろうと踏む。**時刻型は引き金ではなく error の被害報告位置 (colid)**。当初仮説の「`datetimeoffset` の Display が scale 抜きで吐かれている」は別 bug の可能性はあるが本症状の主犯ではない (datetimeoffset 単独 bulk が成立しているため)。タスク: 縮退テスト (列を 1 つずつ抜く) で真因を特定 → 上流 tiberius issue / fork → 修復後に `bulk_all_types_together` の `#[ignore]` を外し、bench-rss prepare も bulk へ復帰させる。短期に bench-rss 1h を縮める手段は別 entry の multi-row VALUES batch を参照
- RDB writer の multi-row `VALUES` batch INSERT — 現状 `LayerWriter::write_batch` は 1 行ごとに 1 RPC を発行する。SQL Server は 1 RPC あたり 2100 param の上限があるので、`chunk_rows = floor((2100 - margin) / params_per_row)` で **自動算出した chunk** を `INSERT INTO t (...) VALUES (?,?,...), (?,?,...), ...` に詰める方式に変更すると round-trip が ~175× 削減される。bench-rss の SQL Server prepare は 1h4m → 8〜15 分の試算 (`docs/STREAMING.md` の SQL Server peak RSS 計測コスト削減)。本番 driver の `--insert-mode=batch` も同時に高速化。実装は `shpx-rdb-common::multirow_chunk_rows(params_per_row, max_params_per_rpc)` を共通ヘルパとして置き、SQL Server / 将来の MySQL (param 上限 65535) / Oracle で再利用。LOB を多く含む schema では payload byte cap (16 MB 目安) を併用
