//! PostGIS reader/writer の往復統合テスト。
//!
//! `SHPX_TEST_PG_URL` 環境変数が設定されている場合のみ実行する（未設定なら
//! `eprintln!` を 1 行出して `return`）。CI では `services.postgis` を立てて
//! `SHPX_TEST_PG_URL=pg://shpx:shpx@localhost:5432/shpx_test` を渡す。
//!
//! ローカル実行例:
//! ```bash
...
fn polygon_and_float_and_timestamp_roundtrip() {
    ...
    ts_b.append_value(micros + 86_400_000_000); // +1 day
...
fn binary_and_date_roundtrip() {
    ...
    d_b.append_value(20_564); // 2026-04-25 (= 56300 - epoch... approx) -- value is illustrative
...
///
/// `chunk_rows` を実測してから「`chunk_rows + 1` 行」を投入することで、
/// 「full chunk + 末尾 remainder 1 行」のパスを通す。chunk loop の境界バグや
/// $N placeholder 番号の連番ズレを検出する。multi-row 化の効果として 1 RPC で
/// 16k 行を流し込むため、行数の割にテスト時間は短い。
#[test]
#[allow(
...
fn multirow_values_chunk_boundary_roundtrip() {
    ...
    // params_per_row = 1 (idx) + 1 (EWKB) = 2。i16::MAX = 32767 / margin 16 から
    // chunk_rows = floor(32751 / 2) = 16375。`chunk_rows + 1` = 16376 行を投入。
    let chunk_rows = shpx_rdb_common::multirow_chunk_rows(2, i16::MAX as usize, 16);
    let total = chunk_rows + 1;

    let mut idx_b = Int32Builder::new();
    let mut geom_b = BinaryBuilder::new();
    for i in 0..total {
        idx_b.append_value(i as i32);
        // Arrow 列は素の WKB を保持する規約。writer 内部で `ewkb::encode_with_srid` が
        // SRID を埋めて bind するため、テスト側は plain WKB を流すだけで良い。
...
fn missing_table_param_errors() {
    // CRUD は走らないので env 不要。URI に ?table が無いと open_read が即エラー。
    // ただし、SHPX_PG_TABLE が偶然 env に設定されていれば成功してしまうので skip する。
    if std::env::var(shpx_driver_postgis::options::ENV_TABLE).is_ok() {
        return;
    }
    let driver = PostgisDriver::new();
